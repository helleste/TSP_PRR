...
void findSuccessors(state parentState) {
	...
	state currentState = {0,{-1,-1,-1,-1,-1},0};
	jobs.pop();
...
int main(int argc, char *argv[]) {
	...
	if (rank == 0)		
	{
		...
		while(1) {
			...
			state state_tbd = {city_id, {0,-1,-1,-1,-1}, calcVal(state_tbd, start)};
			jobs.push(state_tbd);

			/* Count the path */
			while(jobs.size() > 0) {
				//cout << "Processing state: " << jobs.top().id << endl;
				findSuccessors(jobs.top());
			}

		}

		/* Send best case to the master */
		position = 0;
